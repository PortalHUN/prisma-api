...
const BlockAPIKey = async (req, res) => {
  ...
  const key = await db.$queryRaw`SELECT Name FROM api_keys WHERE ID = ${ID} AND Name = ${Name};`;
  ...
  const token = await jwt.sign({ Name: finish.Name, API_key: finish.API_key }, process.env.APIKEYSECRET);

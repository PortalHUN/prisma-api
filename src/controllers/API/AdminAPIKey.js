...
const AdminAPIKey = async (req, res) => {
  ...
  const key = await db.$queryRaw`SELECT Admin FROM api_keys WHERE ID = ${ID} AND Name = ${Name};`;
  ...
  return res.status(200).json({
    message: key[0].Admin ? "Successfully removed Admin privileges" : "Successfully added Admin privileges",
  });
};
